...
class Dtts_peak_finder():
    ...
    def __init__(self,image):
        ...
        Input:
            ...
            - sky: if True, subtract the sky measured in the 7 outermost rows and columns 
        Output:
            - nothing
        """            
        if not image.ndim == 3 or image.shape[1]!=32 or image.shape[2]!=32:
            raise TypeError('The input is not a 32x32 cube.')
        
        self.nframes,self.ny,self.nx = image.shape
        self.mask = np.zeros((32,32),dtype=bool)
        self.mask[7:-7,7:-7]=True
        self.mask[:,0]=True
        self.mask[0,:]=True
        px_scale = 10.e-3 # mas/px
        self.theoretical_fwhm = np.rad2deg(1.5888e-6/8.)*3600/px_scale
        ...
        self.cube = np.copy(image)
        self.residuals = np.zeros_like(image)
        threshold_bck = 4.
        max_cube = np.max(image,axis=(1,2))
        nbck = np.sum(max_cube<threshold_bck)
        print('Number of backgrounds: {0:d}'.format(nbck))
